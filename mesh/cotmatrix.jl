...
function cotmatrix(V,F)
  ...
  if ss == 3
    ...
    if(size(F,1) == 3)
      warn("F seems to be 3 by #F, it should be #F by 3");
    end
    Ft = F';
    ...
    cot12 = -dot2(v1,v2)./dblA/2; cot23 = -dot2(v2,v3)./dblA/2; cot31 = -dot2(v3,v1)./dblA/2;
  ...
  elseif ss == 4
    ...
    #C(abs(C)<10*eps) = 0;
    # add to entries
    L = sparse(vec(F[:,[2 3 1 4 4 4]]),vec(F[:,[3 1 2 1 2 3]]),C,n,n);
    ...
    if(all(diag(L)>0))
      warn("Flipping sign of cotmatrix3, so that diag is negative");
      L = -L;
    end
  end
  L
