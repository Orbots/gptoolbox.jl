...
"""
function arap_linear_block(V::Array{Float64}, F::Array{Int64}, d::Int64; energy = "spokes")
  # number of vertices
  n = size(V,1);
  # number of elements
  m = size(F,1);
  ...
  function spokes_linear_block(V::Array{Float64}, F::Array{Int64}, d::Int64)
    ...
    K = K + sparse(diagm(vec(sum(K,2))));
    K = 0.5*K;
  end

  function spokes_and_rims_linear_block(V::Array{Float64}, F::Array{Int64}, d::Int64)
    # Computes a matrix K such that K * R computes
  ...
  function elements_linear_block(V,F,d)
    # Computes a matrix K such that K * R computes
    #  ∑    -2*cot(aij) * (V(i,d)-V(j,d)) * Raij +
    #       -2*cot(bij) * (V(i,d)-V(j,d)) * Rbij
    # j∈N(i)
  ...
  if energy == "spokes"
    spokes_linear_block(V,F,d);
    elseif energy == "spokes-and-rims"
    spokes_and_rims_linear_block(V,F,d);
  elseif energy ==  "elements"
    elements_linear_block(V,F,d);
  else
    error(string("Unsupported energy type: ", energy));
  end
